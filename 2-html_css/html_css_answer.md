...
<!DOCTYPE html>
```
- HTML이 어떤 버전으로 작성되었는지 미리 선언해 웹브라우저가 내용을 올바로 표시할 수 있도록 하는 것입니다.

**쉽게 설명하자면 문서형식을 정의해주는 것이다.**

### `meta 태그에 대해서 알고 있나요?`
- HTML 태그 중 하나로, 문서 그 자체를 설명하는 태그가 메타 태그입니다.
...
- 웹 표준이란 웹 상에서 표준적으로 사용되는 기술로 웹사이트를 어떠한 운영체제에서나 어떠한 브라우저에서 보더라도 동일하게 보여지도록 W3C(World Wide Web Consortium) 기구 표준에 맞추는 것입니다.

최신 웹 표준 버전은 `HTML5`, `CSS3`입니다.


### `HTML5에서 추가된 내용이 있나요?`
- `canvas` 기능 추가로 자바스크립트를 통해 다양한 그림을 실시간으로 그릴 수 있는 공간을 제공해주며, 모든 디바이스에서 웹페이지 호환이 가능해지고 시멘틱 웹 기술을 지원하기 시작했습니다.

### `웹 접근성이란?`
- 웹 접근성은 **모든 사람 (비장애인, 장애인, 노인 등 )이 차별 없이 웹 사이트를 자유롭게 이용할 수 있게 하는 권리**를 말합니다. 

### `웹 접근성에 맞는 마크업 예시 몇가지 말해보시오`
1. 이미지에 **alt, IR기법을 사용하여 대체텍스트를 제공**
...
### `시멘틱 태그란 무엇인가 왜 사용하는가`
- 의미요소(Semantic Element)는 HTML로 만든 문서에 추가적으로 의미를 부여해 접하는 사람의 이해를 돕고, 검색엔진을 통해 검색이 잘 될 수 있도록 돕습니다.
...
### `Section 태그와 article 태그의 차이점`
- `article`은 문서나 사이트에서 독립된 컨텐츠 영역을 지정합니다. 이 부분을 다른 곳에 옮기더라도 분리되어지고, 의미가 통해야합니다.
...
### `SVG란`
- SVG는 Scalable Vector Graphics라는 뜻인데, 번역하자면 확장가능한 벡터 기반의 그래픽이다. 
...
1. 반응형 웹이란 **하나의 웹사이트에서 접속하는 디스플레이의 종류에 따라 화면의 크기가 자동으로 변하도록 만든 웹페이지 접근 기법을 말합니다.**
2. 적응형 웹은 사용자의 기기 및 해상도의 정보를 미리 받아서 조건에 맞는 화면을 보여주는 것입니다.
